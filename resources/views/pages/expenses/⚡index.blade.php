...
<section class="w-full space-y-6" x-data="{ showExpenseSummary: false, selectedExpense: null }">
    ...
    <flux:modal name="manage-expense-categories" class="md:w-[42rem]">
        <div class="space-y-5">
            <div>
            ...
            <table class="w-full min-w-[980px] text-left text-sm">
                ...
                <tbody>
                    @foreach ($this->expenses as $expense)
                        @php
                            $carLabel = trim(collect([$expense->car->year, $expense->car->make, $expense->car->model])->filter()->implode(' '));
                            $expenseSummary = [
                                'date' => $expense->expense_date->format('d-m-Y'),
                                'category' => $expense->category->name,
                                'amount' => $this->formatCurrency($expense->amount),
                                'vendor' => $expense->vendor ?: __('N/A'),
                                'odometer' => $expense->odometer !== null ? number_format((float) $expense->odometer) : __('N/A'),
                                'tags' => implode(', ', $expense->tags ?? []) ?: __('N/A'),
                                'car' => $carLabel !== '' ? $carLabel : __('N/A'),
                                'notes' => $expense->notes ?: __('N/A'),
                            ];
                        @endphp
                        ...
                        <tr
                            ...
                            tabindex="0"
                            x-on:click='selectedExpense = @json($expenseSummary); showExpenseSummary = true'
                            x-on:keydown.enter='selectedExpense = @json($expenseSummary); showExpenseSummary = true'
                        >
                            ...
                            <td class="px-3 py-2">
                                <div class="flex items-center justify-end gap-2">
                                    <flux:button variant="ghost" size="sm" wire:click="editExpense({{ $expense->id }})" x-on:click.stop>{{ __('Edit') }}</flux:button>
                                    <flux:button variant="danger" size="sm" wire:click="deleteExpense({{ $expense->id }})" x-on:click.stop>{{ __('Delete') }}</flux:button>
                                </div>
                            </td>
                        </tr>
                    @endforeach
    ...
    <div
        x-show="showExpenseSummary"
        x-transition.opacity
        class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4"
        x-on:click.self="showExpenseSummary = false"
        x-on:keydown.escape.window="showExpenseSummary = false"
    >
        <div class="w-full max-w-lg rounded-xl bg-white p-5 shadow-xl dark:bg-zinc-900">
            <div class="flex items-start justify-between gap-3">
                <div>
                    <flux:heading>{{ __('Expense Summary') }}</flux:heading>
                    <flux:subheading x-text="selectedExpense?.date"></flux:subheading>
                </div>
                <flux:button variant="ghost" x-on:click="showExpenseSummary = false">{{ __('Close') }}</flux:button>
            </div>

            <div class="mt-4 grid gap-2 text-sm">
                <div><strong>{{ __('Category') }}:</strong> <span x-text="selectedExpense?.category"></span></div>
                <div><strong>{{ __('Amount') }}:</strong> <span x-text="selectedExpense?.amount"></span></div>
                <div><strong>{{ __('Vendor') }}:</strong> <span x-text="selectedExpense?.vendor"></span></div>
                <div><strong>{{ __('Odometer') }}:</strong> <span x-text="selectedExpense?.odometer"></span></div>
                <div><strong>{{ __('Tags') }}:</strong> <span x-text="selectedExpense?.tags"></span></div>
                <div><strong>{{ __('Car') }}:</strong> <span x-text="selectedExpense?.car"></span></div>
                <div><strong>{{ __('Notes') }}:</strong> <span x-text="selectedExpense?.notes"></span></div>
            </div>
        </div>
    </div>
</section>
